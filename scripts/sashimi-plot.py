...
def define_options():
        ...
        parser.add_argument("-l", "--pos_labels", type=str2bool, nargs='?', default=False, dest="position_labels",
                help="Test labels for gtf Positions: <True> <False> [default=%(default)s]")
...
def read_gtf(f, c):
        ...
        with open(f) as openf:
                for line in openf:
                        ...
                        strand = '"' + strand + '"'
                        #new for add gene name
                        gene_id = d["gene_id"]
                        
                        if "gene-" in gene_id:  #for yeast
                            tmp = gene_id.split("-")
                            del tmp[0]
                            ss = "-"
                            gene_id = ss.join(tmp)
                        ...
                        if el == "transcript":
                                if (el_end > start and el_start < end):
                                        transcripts[transcript_id] = max(start, el_start), min(end, el_end), strand, gene_id
                                continue
                        if el == "exon":
                                if (start < el_start < end or start < el_end < end):
                                        exons.setdefault(transcript_id, []).append((max(el_start, start), min(end, el_end), strand, gene_id))
...
def make_introns(transcripts, exons, intersected_introns=None):
        ...
        introns = OrderedDict()
        #print("its_intron->" + str(intersected_introns))
        if intersected_introns:
            for tx, (tx_start,tx_end,strand, gene_id) in new_transcripts.items():
                total_shift = 0
                #check exon
                #for i, (exon_start,exon_end,strand, gene_id) in enumerate(exons.get(tx,[])):
                #    print (gene_id + "-exon:"+str(exon_start) +"-"+str(exon_end))
                ...
                for a,b in intersected_introns:
                    ...
                    for i, (exon_start,exon_end,strand, gene_id) in enumerate(exons.get(tx,[])):
                        ...
                        if b <= exon_end:
                            new_exon_end = new_exons[tx][i][1] - shift
                        new_exons[tx][i] = (new_exon_start,new_exon_end,strand, gene_id)
                tx_start = min(tx_start, sorted(new_exons.get(tx, [[sys.maxsize]]))[0][0])
                new_transcripts[tx] = (tx_start, tx_end - total_shift, strand, gene_id)

        for tx, (tx_start,tx_end,strand, gene_id) in new_transcripts.items():
            intron_start = tx_start
            ex_end = 0
            for ex_start, ex_end, strand, gene_id in sorted(new_exons.get(tx, [])):
                #print("new_exon:"+str(ex_start)+"-"+str(ex_end))
                intron_end = ex_start
                if tx_start < ex_start:
                        introns.setdefault(tx, []).append((intron_start, intron_end, strand, gene_id))
                intron_start = ex_end
            if tx_end > ex_end:
                introns.setdefault(tx, []).append((intron_start, tx_end, strand, gene_id))

        #for i, (i_start,i_end,strand, gene_id) in enumerate(introns.get(tx,[])):
            #print (gene_id + "-instron:"+str(i_start) +"-"+str(i_end))
        d = {'transcripts': new_transcripts, 'exons': new_exons, 'introns': introns}
        return d


def gtf_for_ggplot(annotation, start, end, arrow_bins, pos_label):
        arrow_space = int((end - start)/arrow_bins)
        #print('arrow_space:' + str(arrow_space))
        #print(str(start) + '-' + str(end) + ':' + str(arrow_bins))
        if arrow_space == 0:    #no 0 value
            arrow_space = 1
        
        #check val
        #e_key = ",".join(annotation["exons"].keys())
        #e_start = ",".join(map(str, (v[0] for vs in annotation["exons"].values() for v in vs)))
        #e_end = ",".join(map(str, (v[1] for vs in annotation["exons"].values() for v in vs)))

        #print("exon:" +e_key+":"+e_start+":"+e_end)
        #i_key = ",".join(annotation["introns"].keys())
        #i_start = ",".join(map(str, (v[0] for vs in annotation["introns"].values() for v in vs)))
        #i_end = ",".join(map(str, (v[1] for vs in annotation["introns"].values() for v in vs)))
        #print("intron:" + i_key+":"+i_start+":"+i_end)
        ...
        if annotation["exons"]:
                ...
                "gene_id" : ",".join(map(str, (v[3] for vs in annotation["exons"].values() for v in vs))),
                })
        ...
        if annotation["introns"]:
            ...
            ann_list[['introns']] = data.table(
                    ...
                    label_s = c(%(intron_start_label)s),
                    strand = c(%(strand)s),
                    gene_id = c(%(gene_id)s)
            )
            # Create data table for strand arrows
            txarrows = data.table()
            y_text = paste(ann_list[['introns']]$gene_id,":", ann_list[['introns']]$tx, sep="")
            ann_list[['introns']]$tx = y_text
            ...
            """ %({
                    ...
                    "intron_start_label" : ",".join(map(str, (v[0]+1 for vs in annotation["introns"].values() for v in vs))),
                    "strand" : ",".join(map(str, (v[2] for vs in annotation["introns"].values() for v in vs))),
                    "gene_id" : ",".join(map(str, (v[3] for vs in annotation["introns"].values() for v in vs))),
        ...
        if pos_label == True:
            s += """
            gtfp = gtfp + geom_text(data=ann_list[['introns']], aes(label=label_s, y=tx, x=start), size=3, vjust = -1.5, hjust=0.8)
            gtfp = gtfp + geom_text(data=ann_list[['introns']], aes(label=end, y=tx, x=end), size=3, vjust = -1.5, hjust=0.2)
            """
        s += """
        }
        if (length(ann_list[['exons']]) > 0) {
            y_exon = paste(ann_list[['exons']]$gene_id,":", ann_list[['exons']]$tx, sep="")
            gtfp = gtfp + geom_segment(data=ann_list[['exons']], aes(x=start, xend=end, y=y_exon, yend=y_exon), size=5, alpha=1)
        }
...
#       args.bam = "/nfs/no_backup/rg/epalumbo/projects/tg/work/8b/8b0ac8705f37fd772a06ab7db89f6b/2A_m4_n10_toGenome.bam"
        ...
        for strand in bam_dict:
                ...
                if args.gtf:
                        ...
                        #print ('main start:' + str(x[0]) + ' | end:'+str(x[-1]))
                        R_script += gtf_for_ggplot(annotation, x[0], x[-1], arrow_bins, args.position_labels)
                ...
                if os.getenv('GGSASHIMI_DEBUG') is not None:
                        ...
                        plot(R_script)
                log_out = open("r_log.R", "w")
                log_out.write(R_script)
                log_out.close()
        exit()
